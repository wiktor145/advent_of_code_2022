class Range:
    ...
    def overlaps(self, other_range) -> bool:
        for element in other_range.range_set:
            if element in self.range_set:
                return True
...
class Pair:
    ...
    def one_overlaps_the_other(self):
        return self.first_range.overlaps(self.second_range)
...
def count_pairs_that_one_range_overlaps_other(pairs_list: list[Pair]) -> int:
    count: int = 0
    for pair in pairs_list:
        if pair.one_overlaps_the_other():
            count += 1
...
if __name__ == '__main__':
    ...
    number_of_containing_pairs: int = count_pairs_that_one_range_overlaps_other(pairs_list)
